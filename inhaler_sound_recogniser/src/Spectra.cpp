...
void
TSpectra::Create(int nBnd, int bSize, int nVecs, int nStr, int samplingRate)
{
    nBands = nBnd;
    bndSize = bSize;
    nVectors = nVecs;
	...
	if (GlobalSettings.SpeechAnalysisParamaters.DelOnly)
    	nStr -= 1;

         // Header for overall sub-band spectra using MainForm
    Header->nSamples = nVectors;
	Header->sampPeriod = 
		GlobalSettings.SpeechAnalysisParamaters.Shift * 1.0e+7 / samplingRate;
    Header->sampSize = (short int)(nStr * nBands * bndSize * sizeof(float));
    Header->parmKind = 9;		// User kind
}

void
TSpectra::Output(std::string fileName, TSpectra* a1, TSpectra* a2)
{
    ...
    // Write HTK header
    if (a1) Header->sampSize += a1->Header->sampSize;
    if (a2) Header->sampSize += a2->Header->sampSize;
   	...
   	for (int n = 0; n < nVectors; n++)
	{
   		for (int b = 0; b < nBands; b++) 
		{
         	...
         	if (a1) Output(outfile, b, n, a1);
         	if (a2) Output(outfile, b, n, a2);
         }
	}
...
void
TSpectra::Output(std::ofstream& outfile, int b, int n, TSpectra* a)
{
   	// Vector/band offset
