#include "SpecAnalysis.h"
 ...
 //  Constrcutor
TSpecAnalysis::TSpecAnalysis()
{
     //  Analysis parameters
    FrmLength = GlobalSettings.SpeechAnalysisParamaters.FrmLength;
    FrmShift = GlobalSettings.SpeechAnalysisParamaters.Shift;
    nFFT = 2;
    ...
    nFFT2 = nFFT / 2;
    nChn = GlobalSettings.SpeechAnalysisParamaters.nChn;
    nBands = GlobalSettings.SpeechAnalysisParamaters.nBands;
    nChnBand = nChn / nBands;
    nDCTBand = GlobalSettings.SpeechAnalysisParamaters.nDCTBand;
    DCFilt = GlobalSettings.SpeechAnalysisParamaters.DCFilt;

     //  Output sub - band spectral streams
    nStrms = 1;
    if (GlobalSettings.SpeechAnalysisParamaters.Delta)
    {
        nStrms = 2;
    }
    if (GlobalSettings.SpeechAnalysisParamaters.DeltaDelta)
    {
        nStrms = 3;
    }
    ...
    if (nDCTBand > 0)
    {
        ...
        for(int k = 0; k < nDCTBand; k++)
        {
            ...
            {
                CosD[k][n] = sqrt(2.0 / nChnBand) * cos(PI * (k + 1) * (2 * n + 1) / (2 * nChnBand));
            }
        }

        if (GlobalSettings.SpeechAnalysisParamaters.Lift > 1)
        {
            int L = GlobalSettings.SpeechAnalysisParamaters.Lift;
...
TSpecAnalysis::~TSpecAnalysis()
{
 ...
 //  Execute
std::vector <AFrame> 
TSpecAnalysis::Execute(WAV& wav, const std::string& fileName)
{
    Rate = wav.SamplingRate;
    ...
    for (int t = 0; t < nVectors; t++)
    {
        ...
        for (int b = 0; b < Fea->nBands; b++)
        {
            ...
            int v0 = (t * Fea->nBands + b) * Fea->bndSize;
             ...
             //  Static spectra
            if (GlobalSettings.SpeechAnalysisParamaters.DelOnly == 0)
            {
                index2 = 0;
 ...
 //  Overall control
void
TSpecAnalysis::SpecAnalysis(WAV& wav)
{
     //  Number of frames
    ...
    for(int i = 0; i < nFrames; i++)
    {
     ...
     //  Log FB spectra
    if (GlobalSettings.SpeechAnalysisParamaters.LogAmp)
    {
        a1 = &u[0];
        for(int i = 0; i < nFrames; i++)
        {
    ...
    int DelWin = GlobalSettings.SpeechAnalysisParamaters.Delta;
    if (DelWin)
    {
        if (Cep)
        {
            DelSpectra(DelWin, &Cep->Vector[0], &Cep->DelVec[0], nBands * nDCTBand, nFrames);
        }
        if (Dfb)
        {
            DelSpectra(DelWin, &Dfb->Vector[0], &Dfb->DelVec[0], nChn - DCFilt, nFrames);    /* Decorrelated FB nChn */ 
        }
        if (Spe)
        {
            DelSpectra(DelWin, &Spe->Vector[0], &Spe->DelVec[0], nChn, nFrames);
        }
    }

     //  Generate delta - delta spectra
    DelWin = GlobalSettings.SpeechAnalysisParamaters.DeltaDelta;
    if (DelWin)
    {
        if (Cep)
        {
            DelSpectra(DelWin, &Cep->DelVec[0], &Cep->DelDelVec[0], nBands * nDCTBand, nFrames);
        }
        if (Dfb)
        {
            DelSpectra(DelWin, &Dfb->DelVec[0], &Dfb->DelDelVec[0], nChn - DCFilt, nFrames);    /* Decorrelated FB nChn */ 
        }
        if (Spe)
        {
            DelSpectra(DelWin, &Spe->DelVec[0], &Spe->DelDelVec[0], nChn, nFrames);
        }
    }

     //  Remove cepstral mean
    if (GlobalSettings.SpeechAnalysisParamaters.RemoveCepstralMean)
	{
        if (Cep)
        {
 ...
 //  Preprocessing
void
TSpecAnalysis::PreProcess(float* x)
{
     //  pre - emphasize
    if (GlobalSettings.SpeechAnalysisParamaters.PreEmp)
    {
     ...
     //  Ham windowing
    if (GlobalSettings.SpeechAnalysisParamaters.Window)
	{
        for(int n = 0; n < FrmLength; n++)
        {
            x[n] *= HamWin[n];
 ...
 //  Log energy
float
TSpecAnalysis::LogEnergy(float* x)
{
    float e = 0.;
    for(int n = 0; n < FrmLength; n++)
    {
 ...
 //  FFT
void
TSpecAnalysis::FFT(float* x, float* s)
{
    ...
    if (GlobalSettings.SpeechAnalysisParamaters.Power == 0)
        for(int k = 0; k < nFFT2; k++)
        {
            s[k] = sqrt(s[k]);
        }
...
void
TSpecAnalysis::FBFilter(float* s, float* u)
{
    for(int n = 0; n < nChn; n++)
    {
        u[n] = 0.;
...
TSpecAnalysis::LogFB(float* u)
{
 ...
 //  DCT
void
TSpecAnalysis::DCT(float* u, float* c)
{
     //  C0
    float c0 = 0.;
    for(int n = 0; n < nChnBand; n++)
    ...
    if (GlobalSettings.SpeechAnalysisParamaters.Lift > 1)
    {
     ...
     //  Use C0
    if (GlobalSettings.SpeechAnalysisParamaters.C0)
    {
 ...
 //  Cepstrum lifter
void
TSpecAnalysis::CepLifter(float* c, int cSize)
{
...
void TSpecAnalysis::DecorFB_1(float* u, float* c)
{
    int k = 0;
...
void TSpecAnalysis::DecorFB_2(float* u, float* c)
{
    int k = 0;
...
void
TSpecAnalysis::NormLogEne(float* e, int nFrames)
{
    float MaxE = e[0];
...
void
TSpecAnalysis::DelSpectra(int DelWin, float* Vec, float* Del, int VecSize, int nVecs)
{
...
void
TSpecAnalysis::MeanRemove(float* Vec, int VecSize, int nVecs)
{
    for(int n = 0; n < VecSize; n++)
    {
        float* v = Vec + n;
...
void
TSpecAnalysis::InitFB()
{
     //  Frequency scale
    FScale = (GlobalSettings.SpeechAnalysisParamaters.Mel)? (double)Rate / (nFFT * 700.) : 0.;

     //  FBank cut - off frequencies (discrete linear and / or mel)
    LCut = 2;
    HCut = nFFT / 2;
    float MLCut = 0, MHCut = (FScale > 0.)? L2M(nFFT / 2 + 1) : nFFT / 2 + 1;
    if (GlobalSettings.SpeechAnalysisParamaters.LowCut > 0)
    {
        LCut = ((double)GlobalSettings.SpeechAnalysisParamaters.LowCut / (double)Rate) * nFFT + 2.5;	 /* Corrected: 16 - 01 - 2002 */ 
        MLCut = (FScale > 0.)? 1127 * log(1 + (double)GlobalSettings.SpeechAnalysisParamaters.LowCut / 700.) : LCut;
        if (LCut < 2)
        {
            LCut = 2;
        }
    }
    if (GlobalSettings.SpeechAnalysisParamaters.HighCut > 0)
    {
        HCut = ((double)GlobalSettings.SpeechAnalysisParamaters.HighCut / (double)Rate) * nFFT + 0.5;
        MHCut = (FScale > 0.)? 1127 * log(1 + (double)GlobalSettings.SpeechAnalysisParamaters.HighCut / 700.) : HCut;
        if (HCut > nFFT / 2)
        {
            HCut = nFFT / 2;
        }
...
void
TSpecAnalysis::EstNoiseSpec(float* vec, int vSize, int nFrms)
{
    ...
    if (GlobalSettings.SpeechAnalysisParamaters.Power)
